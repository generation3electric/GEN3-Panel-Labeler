import { numberingLayout } from './src/panelLayout.js';
...
export async function analyzePanelPhotos({ files, record }) {
  ...
  const layout = numberingLayout(record.panel?.numberingOrigin);
  const numbering = `Technician selected circuit #1 at the ${layout.label.toLowerCase()}. Numbers increase ${layout.bottomUp ? 'upward from the bottom' : 'downward from the top'}. Odd circuits are on the ${layout.oddLeft ? 'left' : 'right'} and even circuits are on the ${layout.oddLeft ? 'right' : 'left'}, as viewed by the technician. Keep left/right photo names tied to physical sides. If printed numbering contradicts this selection, flag the discrepancy for review; do not silently rotate the photos or invent numbers. For a 2-pole breaker use its LOWER numbered circuit N, with the same-side second position N+2; that second position is ${layout.bottomUp ? 'above' : 'below'} N.`;
